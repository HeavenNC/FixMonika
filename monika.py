                        ...
                        if text is None:
                            ...
                            if cmd == "help":
                               ...
                               if msg._from in admin:
                                   ...
                                   client.sendMessage(to,"Kecepatan mengirim pesan {}".format(str(runtime)))
                            elif cmd == "speed":
                               if msg._from in admin:
                                   client.sendMessage(to, "Proccess...")
...
# Pembatas Script #
        ...
        if op.type == 26:
            try:
                ...
                traceback.print_tb(error.__traceback__)
        if op.type == 65:
            print ("[ 65 ] NOTIFIED DESTROY MESSAGE OR IMAGE")
            if settings["unsendMessage"] == True:
                try:
                    at = op.param1
                    msg_id = op.param2
                    if msg_id in msg_dict:
                        if msg_dict[msg_id]["from"]:
                           if msg_dict[msg_id]["text"] == 'Gambarnya dibawah':
                                ...
                                Aditmadzs = client.getContact(msg_dict[msg_id]["from"])
                                zx = ""
                                zxc = ""
                                zx2 = []
        ...
        if op.type == 65:
            print ("[ 65 ] NOTIFIED DESTROY STICKER")
                ...
                try:
                    ...
                    if msg_id in msg_dict1:
                        if msg_dict1[msg_id]["from"]:
                                ginfo = client.getGroup(at)
                                Aditmadzs = client.getContact(msg_dict1[msg_id]["from"])
